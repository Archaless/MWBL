...
Sensors = {'RainwisePortLog' 'SMAST ATI' 'SMAST Gill' 'SMAST DataQ' 'CBC ATI' 'CBC DataQ' 'KZScintillometer' 'NBAirport' 'DPL_SMAST' 'DPL_CBC' 'OnsetHOBO_CBC' 'OnsetHOBO_SMAST' 'Apogee' 'Ecotech_M9003' 'Ambilabs_2WIN' 'AQMesh'};
...
clear last_file
for n=1:length(Sensors)
  disp(['Loading ',Sensors{n},' ...'])
  subdatastreams = [];
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  switch Sensors{n}
  ...
  case 'OnsetHOBO_CBC'
    Sensors{n} = 'OnsetHOBO';
    subdatastreams = {'CBC'};
    DataDir = [baseDir,'OnsetHOBO_CBC/processed/'];
    DataLabels{n} = 'CBC HOBO Tide Range/Water Temp';
  case 'OnsetHOBO_SMAST'
    Sensors{n} = 'OnsetHOBO';
    subdatastreams = {'SMAST'};
    DataDir = [baseDir,'OnsetHOBO_SMAST/processed/'];
    DataLabels{n} = 'SMAST HOBO Tide Range/Water Temp';
  case 'Apogee'
  ...
  if contains(Sensors{n},'PierCam')
    ...
    for nn = 1:length(FList)
      file_date(nn) = get_File_Date(FList(nn).name,Sensors{n},subdatastreams);
    end
    ...
    if plotflag
      ...
      if overrideflag
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if contains(Sensors{n},'MZA_DELTA')
          thisdate_time = [thisdate_time; NaT; datetime(2024,03,23,00,00,00); datetime(2024,10,31,00,00,00)];
        end

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if (contains(Sensors{n},'ATI') || contains(Sensors{n},'Gill') || contains(Sensors{n},'DataQ'))
        ...
        if contains(Sensors{n},'DPL_SMAST')
	...
	%	  				NaT; datetime(2024,05,15,00,00,00); datetime(2024,10,31)];
	  ...
	  %					NaT; datetime(2023,05,09,16,35,30); datetime(2023,09,30)];
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if contains(Sensors{n},'Apogee')
        %  thisdate_time = [thisdate_time; NaT; datetime(2024,07,31,00,00,00); datetime(2024,10,31,00,00,00)];
        end

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if contains(Sensors{n},'Ecotech_M9003')
        %  thisdate_time = [thisdate_time; NaT; datetime(2024,07,31,00,00,00); datetime(2024,10,31,00,00,00)];
        end

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if contains(Sensors{n},'Ambilabs_2WIN')
        %  thisdate_time = [thisdate_time; NaT; datetime(2024,07,31,00,00,00); datetime(2024,10,31,00,00,00)];
        end

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if contains(Sensors{n},'AQMesh')
        %  thisdate_time = [thisdate_time; NaT; datetime(2024,07,31,00,00,00); datetime(2024,10,31,00,00,00)];
        end
      end
...
for n=1:length(Sensors)
  if ~contains(Sensors{n},'PierCam')
    disp([Sensors{n},':  ',last_file(n).name])
  end
end
